...
import random
import pygame
pygame.init() # Vous utilisez pygame pour la lecture du son
...
class PacmanArena(Arena):
    ...
    def __init__(self, width: int, height: int):
        ...
        self._actors = []
        ## Initialisation des caractères du jeu
...
class Bonus(Actor):
    ...
    ## 0 -> visible
    ## 1 -> mangé par PacMan
    ## 2 -> tous les bonus sont déjà consommés, non visibles
...
class Ghost(Actor):
    ...
    def __init__(self, arena, x:int, y:int, color:int):
        ...
        self._gate = False # Lorsque Vrai, le fantôme peut passer par la porte de l'enceinte initiale.
    ...
    def status(self, status: int):
        ...
        ## -1 -> initialisation
        ## 0 -> normal
        ## 1 -> Pac-Man perd la vie
        ## 2 -> Power Pac-Man
        ## 3 -> Power Pac-Man en conclusion
        ## 4 -> mangé par Pac-Man pendant l'état 2/3
    ...
    def pos_init(self): # La position du fantôme est rétablie à sa position initiale.
        self._x, self._y = self._start_x, self._start_y
        self._sprite = [64, 64 + self._color*16]

    def start(self): # Le fantôme commence à bouger
        self._speed = 2
        self._dir = [0, -2]
        self._sprite = [64, 64 + self._color*16]

    def normal(self): # Le fantôme revient à une vitesse normale
        self._speed = 2
        if self._y == 112 and 92 <= self._x <= 128: self._gate = True # S'il est à l'intérieur de l'enceinte initiale, il doit avoir une chance d'en sortir.
    ...
    def stop(self): # Le fantôme s'arrête
        self._speed = 0
        self._dir = [0, 0]

    def runaway(self): # Le fantôme recule, devient bleu et ralentit.
        self._speed = 1
        if self._dir[0] != 0: self._dir[0] = -self._dir[0]/abs(self._dir[0])
        else: self._dir[1] = -self._dir[1]/abs(self._dir[1])
        self._sprite = [144, 64]

    def eaten(self): # Le fantôme devient "petits yeux" et augmente sa vitesse.
        self._speed = 4
        self._gate = True
    ...
    def move(self):
        ...
        ## Le fantôme ne recule que s'il n'y a rien d'autre à faire.
        if len(dirs) == 0: self._dir = [-self._dir[0], -self._dir[1]]
        ## Mode effrayé (mouvements aléatoires)
        elif self._status in [2, 3]: self._dir = random.choice(dirs)
        else:
            ## Selezione dell'obiettivo (xt, yt)
            if self._gate and self._status != 4: # Le fantôme sort de l'enceinte initiale pour entrer dans le jeu.
                xt, yt = 108, 88
                if self._x == xt and self._y == yt:
                    self.status(0)
                    self._gate = False
            elif self._status == 4: # Le fantôme retourne dans l'enclos initial lorsqu'il est mangé par PacMan.
                xt,yt = 108, 112
                if self._x == xt and self._y == yt:
                    self.status(0)
                    self._gate = True
            elif self._behav == 0: # Mode Scatter (patrouille des coins)
                xt,yt = angles[self._color]
                if self._behav_count == FRAME_RATE*7:
                    self._behav_count = 0
                    self._behav = 1
                else: self._behav_count += 1
            elif self._behav == 1: # Mode poursuite (poursuite de Pac-Man)
                for a in self._arena.actors():
            ...
            else: # Calculer la distance la plus courte vers la cible
                x, y = self._x, self._y
                distances = []
...
class PacMan(Actor):
    ...
    def __init__(self, arena, x:int, y:int):
        ...
        self.scores_sprite = []
        self.bonus_sprite = []
        self._bonus = [100, 300, 500, 700, 1000, 2000, 3000, 5000] #valeurs de bonus
        self._gate = False
    ...
    def pos_init(self): # La position de PacMan est restaurée à sa position initiale.
        self._x, self._y = 108, 184
        self._sprite = (16, 16)

    def start(self): # PacMan commence à bouger
        self.pos_init()
        self._dir = (-2, 0)
        self._next_dir = (-2, 0)

    def stop(self): # PacMan s'arrête
        ...
        if isinstance(other, Wall):
            ...
            if (y < self._y + self.H//2 and self._y < y + h//2 and x < self._x + self.W//2 and self._x < x + w//2):
                if other.getStatus() == 0:
                    ...
                    num = 0 # Nombre de fantômes déjà mangés
                    for a in self._arena.actors():
    ...
    def move(self):
        ...
        if self._status == 0:
            ...
            ## Remplacer dir par next_dir si possible
